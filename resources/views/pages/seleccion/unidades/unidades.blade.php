    ...
    >
        <x-slot:acciones>
            @can(App\Enums\Permiso::UnidadesCrear->value)
                <flux:button wire:click="nuevo" variant="primary" icon="plus-sign">Nueva unidad</flux:button>
            @endcan
        </x-slot:acciones>
    </x-pagina.encabezado>
    ...
    <x-panel>
        <div class="flex flex-wrap items-end gap-3">
            ...
            <div class="w-full sm:w-72">
                <flux:input wire:model.live.debounce.300ms="busqueda" icon="search-01" placeholder="Buscar unidad" clearable />
            </div>

            @if ($unidades->isNotEmpty())
                <p class="ms-auto text-sm text-zinc-500 tabular-nums">
            ...
            <flux:table.rows>
                @forelse ($unidades as $unidad)
                    <flux:table.row :key="$unidad->id_uni">
                        <flux:table.cell class="max-w-xs text-sm font-medium whitespace-normal text-zinc-900 dark:text-white">
                            {{ $unidad->nombre_uni }}
                        </flux:table.cell>
                                ...
                                <div class="flex flex-wrap gap-1.5">
                                    @foreach ($unidad->puestos as $puesto)
                                        <flux:tooltip :content="$puesto->nombre_pue.' · '.$puesto->inscripciones_count.' inscrito(s)'">
                                            <flux:badge size="sm" class="tabular-nums">{{ $puesto->codigo_pue }}</flux:badge>
                                        </flux:tooltip>
                                    @endforeach
                                </div>
                            @endif
                        ...
                        <flux:table.cell align="end">
                            <div class="flex justify-end gap-0.5">
                                ...
                                @can(App\Enums\Permiso::UnidadesEditar->value)
                                    <x-tabla.accion wire:click="editar({{ $unidad->id_uni }})" icon="pencil-edit-02" tooltip="Editar" />

                                    <x-tabla.accion
                                        wire:click="alternarEstado({{ $unidad->id_uni }})"
                                        :icon="$unidad->estaHabilitado() ? 'view-off-slash' : 'view'"
                ...
                @empty
                    <x-tabla.vacia
                        :columnas="5"
                        :mensaje="$busqueda === '' ? 'Todavía no hay unidades de organización.' : 'Ninguna unidad coincide con la búsqueda.'"
                        icono="building-03"
                    >
                        @if ($busqueda === '')
                            Se registran solas al importar el listado de postulantes con la columna «UNIDAD DE ORGANIZACIÓN».
                        @endif
                    </x-tabla.vacia>
                @endforelse
            </flux:table.rows>
